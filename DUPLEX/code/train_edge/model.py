...
class ContraNorm(nn.Module):
    def __init__(self, dim, scale=0.1, dual_norm=False, pre_norm=False, temp=1.0, learnable=False, positive=False,
                 identity=False):
        super().__init__()
        # 论文建议 scale 在 0.005 到 0.2 之间，这里默认设为 0.1
    ...
    def forward(self, x):
        # x shape: (Batch, N, Dim) -> 在GNN中通常 Batch=1, N=节点数
        if self.scale > 0.0:
...
class DUPLEX_gat(nn.Module):
    def __init__(self, args):
        ...
        self.am_norms = nn.ModuleList()
        self.ph_norms = nn.ModuleList()

        self.dropout = nn.Dropout(args.dr_rate)
        self.n_layers = args.n_layers


        cn_scale = getattr(args, 'cn_scale', 0.1)
        ...
        self.am_norms.append(ContraNorm(args.hidden_dim, scale=cn_scale))
        self.ph_norms.append(ContraNorm(args.hidden_dim, scale=cn_scale))
    ...
    def forward(self, blocks, input_am, input_ph):
        ...
        for i, block in enumerate(blocks):
            ...
            if i < self.n_layers - 1:
                h_am = h_am.flatten(1)
                h_ph = h_ph.flatten(1)
                h_am = h_am.unsqueeze(0)
                h_am = self.am_norms[i](h_am)
                h_am = h_am.squeeze(0)
                h_ph = h_ph.unsqueeze(0)
                h_ph = self.ph_norms[i](h_ph)
                h_ph = h_ph.squeeze(0)
