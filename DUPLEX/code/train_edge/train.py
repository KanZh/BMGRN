...
for dataset in datasets:
    ...
    for (lw, ld, lr, wd, head) in product(loss_weights, loss_decays, lrs, weight_decays, heads):
        for fusion in fusions:
            for seed in seeds:
                save_log = "head%s_lw%s_ld%s_seed%s_layer%s_fusion%s_use_model%s_l1_%s" % (
                head, lw, ld, seed, n_layers, fusion, use_model, dataset)
                order = "python train_edge/training.py --m %s \
                        ...
                        --use_model %s\
                        --head %s" % (
                save_log, lw, ld, lr, dataset, seed, fusion, save_log, bc_size, n_layers, use_model, head)
                os.system(order)