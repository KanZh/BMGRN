...
import corrresnet_pred_128_mamba as corrresnet_pred
import os
...
for file_name in os.listdir(duplex_directory):
    if file_name.endswith('.npz'):
        ...
        for ki in range(iteration):
            ...
            # positive2_data = np.array(positive2_data)
            # positive1_data = np.array(positive1_data)
            positive2_data = np.array(positive2_data, dtype=object)
            positive1_data = np.array(positive1_data, dtype=object)
            # print("Shape of positive2_data:", positive2_data.shape)
            # print("Shape of positive1_data:", positive1_data.shape)
            # print(type(positive2_data))
            # print(type(positive1_data))
            # print(positive2_data[:5])

            alldata = np.vstack((positive2_data, positive1_data))
            negative0_data = np.array(negative0_data, dtype=object)
            ...
            for train_index, test_index in kf.split(dataX, labelY):
                ...
                trainXX_tf = torch.from_numpy(trainXX[:, :, 0:a]).float().to(device)
                trainXX_target = torch.from_numpy(trainXX[:, :, a:b]).float().to(device)
                trainXX_net_tf_s = torch.from_numpy(trainXX[:, :, b:c]).float().to(device)
                trainXX_net_tf_t = torch.from_numpy(trainXX[:, :, c:d]).float().to(device)
                trainXX_net_target_s = torch.from_numpy(trainXX[:, :, d:e]).float().to(device)
                trainXX_net_target_t = torch.from_numpy(trainXX[:, :, e:f]).float().to(device)

                testXX_tf = torch.from_numpy(testXX[:, :, 0:a]).float().to(device)
                testXX_target = torch.from_numpy(testXX[:, :, a:b]).float().to(device)
                testXX_net_tf_s = torch.from_numpy(testXX[:, :, b:c]).float().to(device)
                testXX_net_tf_t = torch.from_numpy(testXX[:, :, c:d]).float().to(device)
                testXX_net_target_s = torch.from_numpy(testXX[:, :, d:e]).float().to(device)
                testXX_net_target_t = torch.from_numpy(testXX[:, :, e:f]).float().to(device)

                testX_tf = torch.from_numpy(testX[:, :, 0:a]).float().to(device)
                testX_target = torch.from_numpy(testX[:, :, a:b]).float().to(device)
                testX_net_tf_s = torch.from_numpy(testX[:, :, b:c]).float().to(device)
                testX_net_tf_t = torch.from_numpy(testX[:, :, c:d]).float().to(device)
                testX_net_target_s = torch.from_numpy(testX[:, :, d:e]).float().to(device)
                testX_net_target_t = torch.from_numpy(testX[:, :, e:f]).float().to(device)
